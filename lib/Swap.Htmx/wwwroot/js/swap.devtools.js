...
(function () {
     ...
     */
    function dumpState() {
        const stateContainers = document.querySelectorAll('[data-swap-state], [id$="-state"]');
        const state = {};
    ...
    function updatePanelContent(tab) {
        ...
        switch (tab) {
            ...
            case 'state':
                const state = {};
                document.querySelectorAll('[data-swap-state], [id$="-state"]').forEach(container => {
                    state[container.id] = {};
                    container.querySelectorAll('input[type="hidden"]').forEach(input => {
                        state[container.id][input.name || input.id] = input.value;
                    });
                });
                content.innerHTML = `<pre style="margin:0;white-space:pre-wrap;">${JSON.stringify(state, null, 2)}</pre>`;
                break;
            case 'events':
                const events = [...new Set(timeline.filter(e => e.type === 'event' || e.type === 'trigger').map(e => e.name))];
                content.innerHTML = events.map(e => `<div style="padding:4px 8px;border-bottom:1px solid #333;">${e}</div>`).join('');
                break;
        }
    }
    ...
    function initialize() {
        ...
        document.querySelectorAll('[data-swap-state]').forEach(container => {
            if (container.id) {
