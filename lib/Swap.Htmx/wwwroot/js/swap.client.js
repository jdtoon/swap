 ...
 * Handles toasts, SSE reconnection, state coordination, and other client-side interactions.
 * @module SwapClient
 */
(function () {
    'use strict';
    ...
    // ==========================================
    // State Coordination (swap-include-state)
    // ==========================================

    /**
     * Expands swap-include-state attributes to hx-include with the state container's inputs.
     * This allows components to include state without manually listing all field IDs.
     * 
     * Usage:
     * <div swap-include-state="my-state">
     *   <!-- Will auto-include all inputs from #my-state -->
     * </div>
     * 
     * Multiple states:
     * <div swap-include-state="filter-state, sort-state">
     */
    function expandStateIncludes() {
        document.querySelectorAll('[swap-include-state]').forEach(function(element) {
            const stateIds = element.getAttribute('swap-include-state');
            if (!stateIds) return;

            const selectors = stateIds
                .split(',')
                .map(function(id) { return '#' + id.trim(); })
                .join(', ');

            // Set hx-include if not already set, or merge with existing
            const existingInclude = element.getAttribute('hx-include');
            if (existingInclude) {
                element.setAttribute('hx-include', existingInclude + ', ' + selectors);
            } else {
                element.setAttribute('hx-include', selectors);
            }
        });
    }
     ...
     */
    function initialize() {
        // Expand swap-include-state attributes on initial load
        expandStateIncludes();

        // Re-expand after HTMX swaps (in case new elements are added)
    ...
    window.Swap = {
        showToast: showToast,
        expandStateIncludes: expandStateIncludes,
