...
(function () {
    ...
    function initializeOobGuards() {
        document.body.addEventListener('htmx:oobBeforeSwap', function(event) {
            ...
            // The incoming OOB element carries the stamps; fall back across htmx detail shapes.
            var incoming = detail.fragment || detail.newNode || detail.elt || event.target;
            if (!incoming || typeof incoming.getAttribute !== 'function') {
                return;
            }

            var id = incoming.id || (detail.target && detail.target.id);
            if (!id) {
                return;
            }

            // 0. Existence guard (data-swap-if-exists): skip cleanly when the target isn't in the DOM,
            // so AlsoUpdateIfExists never creates a stray element or triggers an htmx no-target error.
            if (incoming.getAttribute('data-swap-if-exists') !== null && !document.getElementById(id)) {
                skipSwap(event);
                return;
            }
            ...
            // 2. Unchanged-content guard (data-swap-hash): skip when the new content fingerprint matches
            // what's already on the target element (avoids needless re-render + lost focus/scroll).
            var hashStr = incoming.getAttribute('data-swap-hash');
            if (hashStr !== null) {
                var current = document.getElementById(id);
                if (current && current.getAttribute('data-swap-hash') === hashStr) {
                    skipSwap(event);
                    return;
                }
            }
    ...
    /** @type {Map<Element, {target: Element, html: string}>} Pre-request snapshots for rollback. */
    ...
    function initializeOptimistic() {
        ...
        document.body.addEventListener('htmx:afterRequest', function(event) {
            ...
            if (snap) {
                // detail.successful is htmx's 2xx success flag; on failure, roll the target back to its
                // own pre-request server-rendered HTML (trusted snapshot, not external input).
