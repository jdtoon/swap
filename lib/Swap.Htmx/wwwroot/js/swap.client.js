...
(function () {
    ...
    // ==========================================
    // OOB Version Guard (data-swap-seq)
    // ==========================================
    ...
    /**
     * Drops stale or duplicate out-of-band swaps using the monotonic data-swap-seq stamp, so an
     * out-of-order or replayed SSE broadcast cannot clobber newer content. Fragments without a
     * data-swap-seq attribute are unaffected.
     */
    function initializeOobGuards() {
        document.body.addEventListener('htmx:oobBeforeSwap', function(event) {
            // @ts-ignore
            var detail = event.detail || {};
            // The incoming OOB element carries the stamp; fall back across htmx detail shapes.
            ...
            if (prev !== undefined && seq <= prev) {
                // Stale or duplicate update — skip the swap.
                // @ts-ignore
