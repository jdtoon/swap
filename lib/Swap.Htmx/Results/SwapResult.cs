...
public sealed class SwapResult : IResult
{
    ...
    public async Task ExecuteAsync(HttpContext httpContext)
    {
        ...
        httpContext.RequestServices.GetService<ISwapDiagnostics>()?.ValidateResponse(_builder);
        ...
        var fragmentRegistry = httpContext.RequestServices.GetService<Swap.Htmx.Fragments.SwapFragmentRegistry>();
        foreach (var oob in Swap.Htmx.Fragments.FragmentResolver.Resolve(
                     fragmentRegistry, _builder.InvalidatedTopics, _builder.OobSwaps.Select(o => o.TargetId), httpContext))
        {
            oobContent.Add(await RenderOobSwapAsync(actionContext, viewData, tempData, oob));
        }
        ...
        if (!string.IsNullOrEmpty(_builder.ViewName) || _builder.Model != null)
        {
            ...
            if (string.IsNullOrEmpty(viewName))
            {
                throw new InvalidOperationException(
                    ...
                    "When using Minimal APIs, there is no MVC action name to infer a view from. " +
        ...
        OobSwap oob)
    {
        ...
        if (!viewResult.Success)
        {
            ...
            var searchPaths = swapOptions?.PartialViewSearchPaths ?? new List<string> { "Shared" };
        ...
        var swapOptions2 = context.HttpContext.RequestServices.GetService<SwapHtmxOptions>();
        if (swapOptions2?.Diagnostics.WarnOnMissingOobTargets == true)
        {
            var logger = context.HttpContext.RequestServices.GetService<ILogger<SwapResult>>();
            logger?.LogWarning(
