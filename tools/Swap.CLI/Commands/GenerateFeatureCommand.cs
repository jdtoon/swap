...
public class GenerateFeatureCommand
{
    ...
    public async Task<int> ExecuteAsync()
    {
        try
        {
            ...
            // Step 4b: Refresh Swap.Events package (critical for --migrate to work)
    ...
    private void GenerateEventConstants(string webProjectDir)
    {
        ...
        // 1. Events.{EntityName}.cs in Swap.Events (partial Events class)
...
";
    ...
    private void RefreshNetMXEventsPackage()
    {
        try
        {
            ...
            // Find Swap.Events project
            var eventsProject = Path.Combine(currentDir, "framework", "Swap.Events", "Swap.Events.csproj");
            if (!File.Exists(eventsProject))
            {
                return; // Not in dev environment, skip
            }
            
            // Repack Swap.Events (WITH build to include new Events.* files)
            ConsoleHelper.WriteInfo($"  🔄 Refreshing Swap.Events package...");
            ...
            {
                // Clear NuGet cache for Swap.Events only
                ...
                ConsoleHelper.WriteSuccess($"  ✓ Swap.Events package refreshed");
            }
            else
            {
                ConsoleHelper.WriteWarning("  ⚠️  Failed to refresh Swap.Events package");
    ...
    /// 1. Events.{EntityName}.cs in Swap.Events project (partial Events class)
    ...
    private void GenerateEventRegistryFiles(string webProjectDir)
    {
        ...
        // 1. Generate Events.{EntityName}.cs for Swap.Events project
        // This file goes in framework/Swap.Events/ directory
    ...
    {
        // Try to find Swap.Events project from current directory
        var currentDir = Directory.GetCurrentDirectory();
        ...
        for (int i = 0; i < 5; i++)
        {
            var frameworkDir = Path.Combine(searchDir, "framework", "Swap.Events");
        ...
        // Strategy 2: Check if we're already in Swap.Events
        if (currentDir.EndsWith("Swap.Events"))
        {
            return currentDir;
        }
        
        ConsoleHelper.WriteWarning("  ⚠️  Could not find Swap.Events project. Skipping Events.{EntityName}.cs generation.");
    ...
    {
        var eventsPartialClass = $@"namespace Swap.Events;
...
";
        ...
        ConsoleHelper.WriteSuccess($"  ✓ Generated Events.{entityName}.cs in Swap.Events");
    }
    ...
    private void GenerateEventDefinitionsClass(string webProjectDir, string entityName, string entityNameLower, string moduleName)
    {
        ...
        var eventDefinitions = $@"using Swap.Events;
...
{{
    ...
    {{
        registry.RegisterEvent(Swap.Events.Events.{entityName}.Created, new EventMetadata
        {{
            Name = Swap.Events.Events.{entityName}.Created,
            Module = ""{moduleName}"",
            Category = ""{entityName}"",
            Direction = Swap.Events.EventDirection.Upstream,
            Description = ""Triggered when a new {entityName} is created. Payload: {{ {entityNameLower}Id: Guid }}""
        }});
        
        registry.RegisterEvent(Swap.Events.Events.{entityName}.Updated, new EventMetadata
        {{
            Name = Swap.Events.Events.{entityName}.Updated,
            Module = ""{moduleName}"",
            Category = ""{entityName}"",
            Direction = Swap.Events.EventDirection.Upstream,
        ...
        registry.RegisterEvent(Swap.Events.Events.{entityName}.Deleted, new EventMetadata
        {{
            Name = Swap.Events.Events.{entityName}.Deleted,
            Module = ""{moduleName}"",
            Category = ""{entityName}"",
            Direction = Swap.Events.EventDirection.Terminal,
...
";
    ...
    private void GenerateEventExtensionMethod(string webProjectDir, string entityName, string moduleName)
    {
        ...
        var extensionMethod = $@"using Swap.Events;
