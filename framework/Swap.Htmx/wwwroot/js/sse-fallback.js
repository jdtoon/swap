...
(function() {
    ...
    class SseConnection {
        ...
        connectSSE() {
            ...
            try {
                const url = this.element.getAttribute('sse-connect') || this.element.getAttribute('hx-sse');
                if (!url) {
                ...
                this.eventSource.onerror = (error) => {
                    ...
                    this.errorCount++;
                    this.triggerEvent('sse:error', { error, errorCount: this.errorCount });

                    if (this.config.enableFallback && this.errorCount >= this.config.fallbackAfterErrors) {
                        this.log('Switching to polling fallback after', this.errorCount, 'errors');
        ...
        scheduleRetry() {
            ...
            const delay = Math.min(1000 * Math.pow(this.config.retryBackoff, this.retryCount), 30000);
            this.retryCount++;
            
            this.log(`Retrying SSE connection in ${delay}ms (attempt ${this.retryCount}/${this.config.maxRetries})`);
        ...
        startPolling() {
            ...
            const pollUrl = this.element.getAttribute('sse-poll') 
            ...
            this.log('Starting polling fallback to:', pollUrl, 'every', this.config.pollInterval, 'ms');
        ...
        handleMessage(eventType, data) {
            ...
            this.triggerEvent(`sse:${eventType}`, { data, usingFallback: this.usingFallback });
        }
    ...
    function initializeSseFallback(container) {
        ...
        elements.forEach(element => {
            ...
            const config = {
                pollInterval: parseInt(element.getAttribute('sse-poll-interval')) || undefined,
                maxRetries: parseInt(element.getAttribute('sse-max-retries')) || undefined,
                timeout: parseInt(element.getAttribute('sse-timeout')) || undefined,
                enableFallback: element.getAttribute('sse-fallback') !== 'false',
                fallbackAfterErrors: parseInt(element.getAttribute('sse-fallback-errors')) || undefined,
            ...
            if (element.getAttribute('sse-connect') || element.getAttribute('hx-sse')) {
                connection.start();
            }
