...
/// <summary>
/// Authorization handler that checks if the user has ANY of the required permissions.
/// Integrates with the PermissionChecker service and includes full observability.
/// </summary>
public class AnyPermissionsAuthorizationHandler : AuthorizationHandler<AnyPermissionsRequirement>
{
    private readonly IPermissionChecker _permissionChecker;
    private readonly ILogger<AnyPermissionsAuthorizationHandler> _logger;
    private static readonly ActivitySource ActivitySource = new("NetMX.Authorization.Handler");

    public AnyPermissionsAuthorizationHandler(
        IPermissionChecker permissionChecker,
        ILogger<AnyPermissionsAuthorizationHandler> logger)
    {
    ...
    {
        using var activity = ActivitySource.StartActivity("HandleAnyPermissionsRequirement");
        ...
        try
        {
            ...
            var hasAnyPermission = await _permissionChecker.IsGrantedAnyAsync(requirement.Permissions);
            ...
            if (hasAnyPermission)
            {
                ...
                _logger.LogInformation(
                    "At least one permission granted from {Permissions} in {DurationMs}ms",
            ...
            {
                _logger.LogWarning(
                    "No permissions granted. Required any of: {Permissions}. Duration: {DurationMs}ms",
                    string.Join(", ", requirement.Permissions), stopwatch.ElapsedMilliseconds);

                activity?.SetTag("authorization.result", "denied_no_permissions");
                activity?.SetTag("duration.ms", stopwatch.ElapsedMilliseconds);
            }
        }
        catch (Exception ex)
